...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings, datadirPath) {
	...
	dcBot.on("message", async message => {
		...
		if (!R.isEmpty(bridges)) {
			...
			if (!antiInfoSpamSet.has(message.channel.id)) {
				...
				message.reply(
					"This is a private bot, bridging a chat in Telegram with one in Discord. " +
						"You've no permission to use it personally anywhere else. " +
						"Still if you've questions then please head towards my master ```@Kazuma Satou#2418```"
				)
	...
	dcBot.on("error", err => {
		if (err.code === "ECONNRESET") {
			...
			logger.warn(
				"Lost connection to Discord's servers. The bot will resume when connection is reestablished, which should happen automatically. If it does not, please report this to Ritesh!"
			);
		} else {
			// Unknown error. Tell the user to tell the devs
			logger.error(
				"The Discord bot ran into an error. Please post the following error message to Master Ritesh."
			);
			logger.error(err);
		}
	});
