	...
	R.ifElse(
		...
		R.when(
			...
			ctx => {
				...
				// Send the reply
				ctx.reply(
					"This is a private project handled and managed by Team WCDI, " +
						" which bridges a chat in Telegram with one in Discord. " +
						"You cannot use it!! strictly prohibited. ",
					{
						parse_mode: "markdown"
					}
				).then(msg =>
	...
	return Promise.resolve()
		...
		.catch(err => {
			if (err.response && err.response.description === "Bad Request: file is too big") {
				ctx.reply("<i>File is too big to handle</i>", { parse_mode: "html" });
			}
		});
}
